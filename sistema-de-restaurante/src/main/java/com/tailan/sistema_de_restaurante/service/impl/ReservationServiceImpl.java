...
import com.tailan.sistema_de_restaurante.exceptions.ReservationIndisponivelException;
import com.tailan.sistema_de_restaurante.exceptions.ReservationNotFoundException;
...
public class ReservationServiceImpl implements ReservationService {
    ...
    public void cancelReserva(UUID reservaId) {
        ...
        mesaService.atualizarStatusMesaDisponivel(reserva.getId());
    ...
    public void validarHorarioReserva(LocalDateTime dataReserva){
        ...
        if (dataReserva.isBefore(agora)){
            throw new ReservationIndisponivelException("A reserva precisa ser para um horário futuro.");
        }

        int hora = dataReserva.getHour();
        if (hora < 0 || hora >= 23){
            throw new ReservationIndisponivelException("A reserva deve ser feita dentro do horário de funcionamento (11:00 às 22:00).");
    ...
    public void validarMesaDisponivelHorarioReserva(UUID mesaId, LocalDateTime dataReserva){
        ...
        if (existeReserva){
            throw new MesaIndisponivelException("Mesa já reservada nesse horário.");
        }
    }
    ...
    private Reservation getReservaByUser(UUID id, User user){
        ...
        Reservation reserva = reservationRepository.findByUserAndId(user, id)
                .orElseThrow(() -> new ReservationNotFoundException("Nenhuma reserva encontrada."));
        return reserva;
    }
